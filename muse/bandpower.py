...
# NumPy renamed ``trapz`` to ``trapezoid`` in 2.0.  MuseLSL and parts of the
# scientific Python stack still require NumPy 1.x, so support both spellings
# instead of forcing an incompatible NumPy upgrade.
...
def compute_band_powers(window, fs):
    ...
    for j, (lo, hi) in enumerate(BANDS.values()):
        ...
        absolute[:, j] = _trapezoid(psd[:, mask], freqs[mask], axis=1)
