...
**`stream_eeg.py` discards ~99.6% of the data.** It pulls one sample per
second from a 256 Hz stream (`max_samples=1` plus `await asyncio.sleep(1)`).
Band power cannot be computed from that — 1 Hz sampling cannot represent an
8 Hz rhythm. It needs to pull continuously and buffer. This is the recording
path, so fixing it is a prerequisite for the item below.
...
**The Unity side is uncompiled.** `Assets/MXFocus/Scripts/` contains
`FocusClient.cs` and `PenWobble.cs`, written without access to a Unity
install — never compiled, never run. They also need scene wiring and an edit
to `MxInkHandler.cs` that only someone with the Editor can do. See that
commit's message for specifics.
